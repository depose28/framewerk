...
The visual design of Framewerk is modeled on neuroscience imaging, not AI marketing aesthetics. Every element -- from node color to bloom parameters to particle behavior -- is specified precisely. This page documents the full visual system.

---

## Design Philosophy

Three principles guide every visual decision in Framewerk:
...
Every color in the visual system is defined in a single file: `lib/constants.ts`. This is the single source of truth. No colors are defined inline in components, shaders, or styles. If you want to modify any color in Framewerk, change it in `constants.ts` and it propagates everywhere.
