...
Framewerk uses a single Zustand store for all application state. This page documents the store structure, key actions, and the reasoning behind the state management architecture.
...
Framewerk avoids this entirely. Any component at any depth can read or update state by importing `useGraphStore`. The Zustand store acts as a global state bus.
