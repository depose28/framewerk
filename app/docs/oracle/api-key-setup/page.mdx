...
The Oracle requires an Anthropic API key to function. This page walks you through obtaining a key, configuring it in Framewerk, and understanding how it is stored and used.
...
4. Click **Create Key**.
5. Give it a name (e.g., "Framewerk") and copy the key. It starts with `sk-ant-`.
...
## Entering Your Key in Framewerk
...
3. Framewerk immediately validates your key by making a small test API call.
...
```
Your browser → Framewerk API route (/api/oracle) → Anthropic API → Response
```

The API key is sent from your browser to Framewerk's API route (which runs on the same server or Vercel function as the rest of the app). The API route uses the key to authenticate with Anthropic's API and returns the response. At no point is the key stored server-side or transmitted to a third party.
...
If you are self-hosting Framewerk, you have an additional option: setting the API key as a server-side environment variable.
...
- The Oracle tab is always visible. If you do not see it, check that you are using the latest version of Framewerk.
