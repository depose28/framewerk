...
Get Framewerk running locally in under two minutes. No database, no external services (except optionally Anthropic for the Oracle), no complex configuration.
...
```bash
git clone https://github.com/your-org/framewerk.git
cd framewerk
```
...
On first load, Framewerk performs a boot sequence: disciplines light up one by one across the graph, and then the full 3D visualization appears. All 700 nodes are rendered as grey spheres in 3D space, with thin dendrite connections between them.
...
Framewerk requires Node.js 18+. If you see syntax errors or module resolution failures, check your Node version:
...
- [Configuration](/docs/self-hosting/configuration) -- Environment variables and customization options.
- [Deployment](/docs/self-hosting/deployment) -- Deploy to Vercel or other platforms.
