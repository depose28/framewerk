...
Framewerk exposes its full graph of 700 mental models as tools via the [Model Context Protocol](https://modelcontextprotocol.io/) (MCP). Any MCP-compatible AI agent -- Claude Desktop, Claude Code, Cursor, and others -- can query the graph directly from within a conversation. Search models, explore connections, filter by discipline, and run Oracle analyses, all without leaving your AI assistant.

---

## What This Enables

The MCP server turns Framewerk into a reasoning layer for AI assistants. Instead of manually browsing the graph in a browser, you can ask your AI assistant questions like:
...
The assistant calls the appropriate Framewerk tools behind the scenes and weaves the results into its response. The 700 models and 2,796 connections become part of the assistant's working knowledge for the duration of your conversation.
...
```bash
git clone https://github.com/depose28/framewerk.git
cd framewerk/packages/mcp-server
npm install
npm run build
```
...
```json
{
  "mcpServers": {
    "framewerk": {
      "command": "node",
      "args": ["/path/to/framewerk/packages/mcp-server/dist/index.js"]
    }
  }
}
```

Replace `/path/to/framewerk` with the actual path where you cloned the repository.
...
Restart Claude Desktop after saving the configuration. You should see "framewerk" appear in the MCP tools list when you start a new conversation.
...
```bash
claude mcp add framewerk node /path/to/framewerk/packages/mcp-server/dist/index.js
```

Replace `/path/to/framewerk` with your clone location. The server is available immediately in your next Claude Code session.

### Other MCP Clients

Any client that implements the MCP specification can connect to the Framewerk server. Point it at the built server entry point (`packages/mcp-server/dist/index.js`) using `node` as the command. Consult your client's documentation for the exact configuration format.
...
> "Give me an overview of all Game Theory models in Framewerk."
...
The MCP server bundles the complete Framewerk dataset:
...
The bundled data is identical to what the Framewerk web application uses. Model IDs, names, and relationships are consistent between the MCP server and the browser-based graph.

---

## Next Steps

- [Oracle Overview](/docs/oracle/overview) -- Learn how the Oracle analysis works and how results are structured.
- [The Graph: Connections](/docs/the-graph/connections) -- Understand the 8 edge types and how they were generated.
- [Architecture Overview](/docs/architecture/overview) -- Technical details on the data pipeline and graph computation.
